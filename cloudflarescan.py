...
class CloudflareScanUI(QWidget):
    ...
    def init_ui(self):
        ...
        self.input_gist_id.setPlaceholderText("Gist ID (留空新建，填写则覆盖 cfs.csv)")
    ...
    def save_results_to_gist(self):
        ...
        try:
            if gist_id:
                self.status_display.append("正在更新 Gist（覆盖 cfs.csv）...")
                ...
                self.status_display.append("正在保存到 Gist（新建 cfs.csv）...")
                description = f"CloudflareScan 测速结果 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                payload = {
                    "description": description,
                    "public": False,
                    "files": {filename: {"content": csv_text}},
                }
            ...
            if new_id and not gist_id:
                ...
                self.status_display.append(f"Gist ID 已填入，下次保存将覆盖该 Gist 的 cfs.csv")
            if new_id:
