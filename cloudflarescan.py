...
class SpeedTestWorker(QThread):
    ...
    def run(self):
        try:
            ...
            def build_speed_result(ip_info: Dict, download_speed: float, test_type: str) -> Dict:
                ip = ip_info.get('ip', '')
                ...
                colo = (colo or 'UNKNOWN').upper()
                return {
                    'ip': ip,
                    'latency': latency,
                    'download_speed': download_speed,
                    'iata_code': colo,
                    'chinese_name': AIRPORT_CODES.get(colo, '未知地区') if colo else '未知地区',
                    'test_type': test_type,
            ...
            if self.region_code:
                ...
                self.status_message.emit(f"开始指定测速：{self.region_code} ({AIRPORT_CODES.get(self.region_code, '未知地区')}) (端口: {self.current_port})")
                ...
                for i, ip_info in enumerate(target_ips):
                    ...
                    speed_result = build_speed_result(ip_info, download_speed, test_type)
            ...
            if self.auto_region_mode:
                ...
                # 为每个地区取“延迟最低”的候选进行测速（最多 10 个/地区）
                candidates_by_region: Dict[str, List[Dict]] = {}
                total_candidates = 0
                for code, items in groups.items():
                    items_sorted = sorted(items, key=lambda x: x.get('latency', float('inf')))
                    candidates = items_sorted[:min(10, len(items_sorted))]
                    if candidates:
                        candidates_by_region[code] = candidates
                ...
                self.status_message.emit(f"{test_type}：将对 {len(candidates_by_region)} 个地区共 {total_candidates} 个候选IP进行测速（每区最多 10 个候选，输出每区 Top3）")

                completed = 0
                speed_results: List[Dict] = []
                for code, candidates in sorted(candidates_by_region.items(), key=lambda x: x[0]):
                    if not self.running:
                        break
                    region_name = AIRPORT_CODES.get(code, '未知地区') if code else '未知地区'
                    self.status_message.emit(f"---- 地区 {code}（{region_name}）：候选 {len(candidates)} 个 ----")

                    region_speed_results: List[Dict] = []
                    for ip_info in candidates:
                        if not self.running:
                            break
                        completed += 1
                        ip = ip_info.get('ip', '')
                        latency = ip_info.get('latency', 0)
                        self.status_message.emit(f"[{completed}/{total_candidates}] 正在测速 {ip} (延迟: {latency}ms) (端口: {self.current_port})")
                        self.progress_update.emit(completed, total_candidates, 0)
                        download_speed = self.download_speed(ip, self.current_port)
                        speed_result = build_speed_result(ip_info, download_speed, test_type)
                        region_speed_results.append(speed_result)
                        self.status_message.emit(f"  测速结果: {download_speed} MB/s")

                        # 不同候选之间稍作间隔
                        for _ in range(self.download_interval * 5):
                            if not self.running:
                                break
                            time.sleep(0.1)

                    # 每区保留 Top3
                    region_speed_results.sort(key=lambda x: x.get('download_speed', 0), reverse=True)
                    top3 = region_speed_results[:3]
                    speed_results.extend(top3)
                    self.status_message.emit(f"地区 {code} Top3 已选出：{len(top3)} 个")

                # 汇总结果按“地区 + 速度”排序，便于输出
                speed_results.sort(key=lambda x: (x.get('chinese_name', ''), x.get('iata_code', ''), -(x.get('download_speed', 0) or 0)))
                if speed_results:
                    self.status_message.emit(f"{test_type}完成：输出 {len(speed_results)} 条结果（每区最多 3 条）")
                else:
                    self.status_message.emit("所有IP测速失败")
                self.speed_test_completed.emit(speed_results)
                return

            # 完全测速：不分区，取全局延迟最低的前 10 个测速
...
class CloudflareScanUI(QWidget):
    ...
    def init_ui(self):
        ...
        self.input_gist_id = QLineEdit()
        self.input_gist_id.setFixedHeight(BTN_H)
        self.input_gist_id.setFont(FONT_BTN)
        self.input_gist_id.setPlaceholderText("Gist ID (留空新建，填写则覆盖 cfs.txt)")
        self.input_gist_id.setStyleSheet(f"""
            QLineEdit {{
                background: white;
                border: 1px solid #D1D5DB;
                border-radius: 6px;
        ...
        """)
        row4.addWidget(self.input_gist_id, 1)
    ...
    def build_gist_txt_text(self, results: List[Dict]) -> str:
        """生成 Gist 用 TXT 内容：每行  ip#地区码#下载速度"""
        lines = []
        for r in results:
            ip = str(r.get("ip", "")).strip()
            iata = str(r.get("iata_code", "")).strip() or "UNKNOWN"
            speed = r.get("download_speed")
            if speed is None:
                speed_str = "0"
            else:
                speed_str = f"{float(speed):.2f}"
            lines.append(f"{ip}#{iata}#{speed_str}")
        return "\n".join(lines) + "\n"
    ...
    def save_results_to_gist(self):
        ...
        gist_text = self.build_gist_txt_text(self.speed_results)
        filename = "cfs.txt"
        gist_id = self.input_gist_id.text().strip()
        ...
        try:
            if gist_id:
                self.status_display.append("正在更新 Gist（覆盖 cfs.txt）...")
                payload = {"files": {filename: {"content": gist_text}}}
                url = f"https://api.github.com/gists/{gist_id}"
                req = urllib.request.Request(url, data=json.dumps(payload).encode("utf-8"), method="PATCH", headers=headers)
            else:
                self.status_display.append("正在保存到 Gist（新建 cfs.txt）...")
            ...
            if html_url:
                ...
                self.status_display.append("Gist 操作成功，但未返回链接")
                self.status_label.setText("Gist 已保存")

            if new_id and not gist_id:
                self.input_gist_id.setText(new_id)
                self.status_display.append(f"Gist ID 已填入，下次保存将覆盖该 Gist 的 cfs.txt")
            if new_id:
                self.status_display.append(f"Gist ID: {new_id}")
